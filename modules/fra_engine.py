...
def analyze_reliability(flight_iata, api_key):
    ...
    if not flight_data:
        return {"error": "Flight not found in Aviation Edge database."}
    ...
    # (In V2 we will use Regex for exact parsing, this is a keyword scan for speed)
    if "TS" in taf_raw: 
        score -= 30
    ...
    elif flight_data['status'] == 'incident':
        score = 0

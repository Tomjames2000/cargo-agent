...
class LogisticsTools:
    def __init__(self):
        self.geolocator = Nominatim(user_agent="cargo_app_prod_v28_final", timeout=10)
        self.AIRPORT_DB = {
    ...
    def search_flights(self, origin, dest, date, show_all_airlines=False):
        ...
        try:
            ...
            for f in raw[:20]:
                ...
                # Parse Full Datetimes (Important for Next Day delivery)
                dep_full = legs[0].get('departure_airport', {}).get('time', '') # "2025-10-14 10:00"
                arr_full = legs[-1].get('arrival_airport', {}).get('time', '')  # "2025-10-14 16:00"
                ...
                results.append({
                    ...
                    "Dep Time": dep_full.split()[-1], # HH:MM
                    "Dep Full": dep_full,             # Full Datetime for logic
                    "Dest": legs[-1].get('arrival_airport', {}).get('id', 'UNK'),
                    "Arr Time": arr_full.split()[-1], # HH:MM
                    "Arr Full": arr_full,             # Full Datetime for logic
                    "Duration": f"{f.get('total_duration',0)//60}h {f.get('total_duration',0)%60}m",
                    "Conn Apt": conn_apt,
                    "Conn Time": conn_time_str,
                    "Conn Min": conn_min # Store integer for filtering
                })
            return results
...
with st.sidebar:
    ...
    del_time = datetime.time(18, 0) # Default
    del_date_obj = None # Default
    
    if has_deadline:
        # NEW: Delivery Date Selector
        del_date_obj = st.date_input("Delivery Date", datetime.date.today() + datetime.timedelta(days=1))
        del_time = st.time_input("Must Arrive By (Time)", datetime.time(18, 0))
...
if run_btn:
    ...
    with st.status("Running Logistics Engine...", expanded=True) as status:
        ...
        earliest_dep_str = earliest_dep_dt.strftime("%H:%M") # For display/filtering
        
        # Latest Arr (Full Datetime Calculation)
        full_deadline_dt = None
        total_post = 0
        
        if has_deadline and del_date_obj:
            del_drive_used = max(d2['time_min'], custom_d_buff)
            total_post = del_drive_used + 60
            
            # Combine User Selected Delivery Date + Time
            full_deadline_dt = datetime.datetime.combine(del_date_obj, del_time)
            
            # Subtract buffers to find latest flight arrival
            latest_arr_dt = full_deadline_dt - datetime.timedelta(minutes=total_post)
        ...
        for day_obj in days_to_search:
            ...
            for f in raw_data:
                ...
                # Check 3: Arrival Deadline (Smart Date Check)
                if full_deadline_dt and not reject_reason:
                    # Parse the API's full string "2025-10-14 16:00"
                    try:
                        flight_arr_dt = datetime.datetime.strptime(f['Arr Full'], "%Y-%m-%d %H:%M")
                        if flight_arr_dt > latest_arr_dt:
                            reject_reason = f"Arrives Too Late ({f['Arr Time']})"
                    except:
                        pass # If parsing fails, skip strict check to be safe
    ...
    with col1:
        ...
        * **Buffer Logic:** MAX({d1['time_min']}, {custom_p_buff}) + 60 = **{total_prep} min**
        * **Earliest Dep:** {earliest_dep_str}
        """)
    ...
    with col2:
        ...
        if has_deadline:
            st.markdown(f"""
            * **Deadline:** {del_date_obj.strftime('%m/%d')} at {del_time.strftime('%H:%M')}
            * **Drive Mileage:** {d2['miles']} miles
            * **Drive Time:** {d2['time_str']}
            * **Buffer Logic:** MAX({d2['time_min']}, {custom_d_buff}) + 60 = **{total_post} min**
            * **Must Arrive By:** {latest_arr_dt.strftime('%H:%M')}
            """)
        else:
